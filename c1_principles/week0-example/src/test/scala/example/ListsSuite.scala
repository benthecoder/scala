...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum of list with zeros (10pts)") {
    assert(sum(List(0, 0, 0)) == 0)
  }

  test("max of list with repeated elements (10pts)") {
  ...
  test("max of list with zeros (10pts)") {
    assert(max(List(0, 0, 0)) == 0)
  }

  test("max of list with negative numbers (10pts)") {
    assert(max(List(-1, -2, -3)) == -1)
  }

  test("max of list with negative and positive numbers (10pts)") {
